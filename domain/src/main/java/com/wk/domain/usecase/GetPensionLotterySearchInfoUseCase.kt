...
class GetPensionLotterySearchInfoUseCase @Inject constructor(
    private val lotteryInfoRepository: LotteryInfoRepository
) {
    suspend operator fun invoke(type: String, round: String): Result<LotteryInfoModel> =
        withContext(Dispatchers.IO) {
            lotteryInfoRepository.getPensionLotterySearchInfo(type, round)
        }
}